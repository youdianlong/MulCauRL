...
class Bert(nn.Module):
    ...
    def __init__(self, tokenizer, lan):
        ...
        if lan == 'DistilBERT':
            ...
            config = BertConfig.from_pretrained("/data1/zyc/project/causal1_breast/dataset_modules/Clinical-BERT/", output_hidden_states=True)
            self.bert = BertModel.from_pretrained("/data1/zyc/project/causal1_breast/dataset_modules/Clinical-BERT/", config=config)
        elif lan == 'RoBERTa':
            from transformers import RobertaModel, RobertaConfig
            config = RobertaConfig.from_pretrained("/data1/zyc/project/causal1_breast/dataset_modules/Clinical-BERT/", output_hidden_states=True)
            self.bert = RobertaModel.from_pretrained("/data1/zyc/project/causal1_breast/dataset_modules/Clinical-BERT/", config=config)
        elif lan == 'DeBERTa':
            from transformers import DebertaConfig, DebertaModel
            config = DebertaConfig.from_pretrained("/data1/zyc/project/causal1_breast/dataset_modules/Clinical-BERT/", output_hidden_states=True)
            self.bert = DebertaModel.from_pretrained("/data1/zyc/project/causal1_breast/dataset_modules/Clinical-BERT/", config=config)
    ...
    def forward(self, tokens): #, seq_len, seg_feats, seg_num):
        input_ids = tokens["input_ids"].to(self.bert.device)
        attention_mask = (input_ids != self.tokenizer.pad_token_id).float().to(input_ids.device)
