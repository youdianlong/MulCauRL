...
from models.model_clam_lcp import CLAM_MB, CLAM_SB
...
if __name__ == '__main__':
    ...
    if 'multimodal_arguments' in args:
        multimodal_args = argparse.Namespace(**args['multimodal_arguments'])
    else:
        multimodal_args = argparse.Namespace(text_dir=None, gene_dir=None, use_text=False, use_gene=False)
    ...
    if model_args.initiate_fn == 'initiate_model':
        model =  initiate_model(model_args, ckpt_path,multimodal_args=multimodal_args)
    else:
        raise NotImplementedError
    ...
    for i in tqdm(range(len(process_stack))):
        ...
        heatmap_save_name = '{}_{}_roi_{}_blur_{}_rs_{}_bc_{}_a_{}_l_{}_bi_{}_{}_causal.{}'.format(slide_id, float(patch_args.overlap), int(heatmap_args.use_roi),
